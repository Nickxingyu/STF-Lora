...
class SymmetricalTransFormer(CompressionModel):
    ...
    def forward(self, x):
        ...
        x_hat = self.end_conv(
            y_hat.view(-1, Wh, Ww, self.embed_dim).permute(0, 3, 1, 2).contiguous()
        )
        return {
            "x_hat": x_hat,
    ...
    def load_state_dict(self, state_dict, strict: bool = True, update: bool = True):
        if update:
...
class SymmetricalTransFormerWithLoRA(SymmetricalTransFormer):
    ...
    def from_state_dict(
        ...
        """Return a new model instance from `state_dict`."""
        net = cls(lora_r=lora_r, hyper_lora_r=hyper_lora_r, merge_weights=merge_weights)
        net.load_state_dict(state_dict, strict)
        return net
